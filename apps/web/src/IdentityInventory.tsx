...
export function IdentityInventory({ user, selectedId, onSelect, label = "Identity inventory", isSelectable = () => true }: {
  user: User; selectedId: string; onSelect: (identity: Identity) => void;
  label?: string; isSelectable?: (identity: Identity) => boolean;
}) {
  ...
  return <section className="identity-list" aria-label={label} aria-busy={loading}>
    <form className="inventory-search" onSubmit={(event) => {
      ...
      <label htmlFor={searchId}>{label}: search all tenant identities</label>
      <input id={searchId} className="search" value={draft} maxLength={255} onChange={(event) => setDraft(event.target.value)} placeholder="Name, email, department or source" />
    ...
    {!loading && page && <>
      ...
      {page.items.map((identity) => <button key={identity.id} disabled={!isSelectable(identity)} className={identity.id === selectedId ? "identity-row selected" : "identity-row"} aria-pressed={identity.id === selectedId} onClick={() => onSelect(identity)}>
      ...
      {selectedId && !page.items.some((item) => item.id === selectedId) && <p>The selected identity is outside this results page.</p>}
